...
ВАЖНО: В поле categories укажи ТОЛЬКО 1-6 наиболее подходящие категории. НЕ перечисляй все возможные категории — только те, которым компания реально соответствует.
...
def parse_supplier_from_url(
    ...
    if existing_categories:
        cat_hint = (
            "\n\nВот список существующих категорий в системе. Выбери МАКСИМУМ 6 "
            "наиболее подходящие категории ТОЛЬКО из этого списка:\n"
            + ", ".join(existing_categories)
            + "\nНе придумывай новых категорий и НЕ включай все категории подряд."
        )
